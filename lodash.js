...
// use map to get addresses here.
const myEmployeesAddresses = _.map(myEmployees, (e, i, wholeObj) =>  
{
  return e.address;
});
...
const myEmployeesAges = _.map(myEmployees, (e, i, wholeObj) =>
{
  return e.age;
}); 
...
console.log('purchasesByMonth = ', purchasesByMonth);

// Bonus Points
// Use the grouped purchasesByMonth and reduce to create a totalByMonth object.
// const totalByMonth = _.groupBy(purchasesByMonth, _.reduce(purchasesByMonth.price, function (val, i){ val[i] += val[i] }, 0), 'totalPrice');
